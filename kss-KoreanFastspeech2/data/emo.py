...
def prepare_align(in_dir, meta):
    with open(os.path.join(in_dir, meta), encoding='utf-8') as f:
        for line in f:
            ...
            basename=os.path.basename(basename.replace('.wav','.txt'))
...
def build_from_path(in_dir, out_dir, meta):
    ...
    with open(os.path.join(in_dir, meta), encoding='utf-8') as f:
        for index, line in enumerate(f):
            ...
            ret = process_utterance(in_dir, out_dir, basepath, scalers)
...
def process_utterance(in_dir, out_dir, basepath, scalers):
    wav_bak_path = basepath
    basename = os.path.basename(basepath)
    basename = basename.replace('.wav', '')
    wav_bak_path = basepath # 기존 wave path (transcript.txt에 저장된)
    wav_path = os.path.join(in_dir, 'wavs', '{}.wav'.format(basename)) # pcm encoding된 waveform이 저장될 곳

    tg_path = os.path.join(out_dir, 'TextGrid', '{}.TextGrid'.format(basename)) 
    if not os.path.exists(tg_path):
        return None

    # Convert emo data into PCM encoded wavs
